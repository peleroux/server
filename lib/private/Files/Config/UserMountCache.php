...
class UserMountCache implements IUserMountCache {
	...
	public function registerMounts(IUser $user, array $mounts, ?array $mountProviderClasses = null) {
		...
		foreach ($newMounts as $mountKey => $newMount) {
			$useMount = !is_array($mountProviderClasses) || in_array($newMount->getMountProvider(), $mountProviderClasses);
			if (!isset($cachedMounts[$mountKey]) && $useMount) {
	...
	public function getMountsForStorageId($numericStorageId, $user = null) {
		...
		while (true) {
			$query->setFirstResult(count($mounts));
			$chunk = $this->fetchMountInfo($query->executeQuery());
			$mounts += $chunk;
			if (count($chunk) < 1000) {
				return $mounts;
			}
	...
	public function getMountsForFileId($fileId, $user = null) {
		...
		$query = $builder->select('id', 'storage_id', 'root_id', 'user_id', 'mount_point', 'mount_id', 'f.path', 'mount_provider_class')
			...
			->andWhere($builder->expr()->orX(
				...
				$builder->expr()->eq('root_id', $builder->createNamedParameter($fileId, IQueryBuilder::PARAM_INT)),
				$builder->expr()->eq('path', $builder->createNamedParameter('')),
				$builder->expr()->eq(
					$builder->func()->concat('path', $builder->createNamedParameter('/')),
					$builder->func()->substring(
						$builder->createNamedParameter($internalPath),
						$builder->createNamedParameter(0, IQueryBuilder::PARAM_INT),
						$builder->func()->add(
							$builder->func()->charLength('path'),
							$builder->createNamedParameter(2, IQueryBuilder::PARAM_INT),
						),
					),
				),
			));
